...
export function openAgentInitialization(options) {
  ...
  const storageSlot = "exocortex.initialize.v1." + options.service + "." + options.component;
  let hint; try { hint = JSON.parse(localStorage.getItem(storageSlot) || "null"); } catch { /* Server jobs are authoritative. */ }
  ...
  function remember(value) {
    hint = value;
    try { if (value) localStorage.setItem(storageSlot, JSON.stringify(value)); else localStorage.removeItem(storageSlot); } catch { /* No secrets in this hint. */ }
  }
  function showJob() {
