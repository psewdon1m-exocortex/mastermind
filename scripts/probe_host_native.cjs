...
(async()=>{
  const root=path.resolve(__dirname,'..'),clean=process.argv.includes('--clean-host');
  const host=clean?'mastermind-clean-install-host':'mastermind-qualification-host',port=clean?18446:18445;
  const prefix=clean?'clean-host':'host';
  ...
  try{
    ...
    await page.screenshot({path:path.join(root,'artifacts/'+prefix+'-native-initial.png')});
    if(process.argv.includes('--trust-generated-vault')){
      const inspection=execFileSync('docker',['exec',host,'docker','exec','mastermind-runtime-1','python','-c',
      ...
      await page.mouse.click(1085,700);
      await page.waitForTimeout(4000);
      ...
      await page.screenshot({path:path.join(root,'artifacts/'+prefix+'-native-trusted.png')});
