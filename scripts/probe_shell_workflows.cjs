...
(async()=>{
  ...
  try{
    ...
    async function nav(name){await page.locator('.sidebar').getByRole('link',{name,exact:true}).click();await page.getByRole('heading',{name:name.toLowerCase(),exact:true}).waitFor();}
    ...
    await nav('Shared');await page.getByRole('button',{name:'Create Share',exact:true}).click();await page.getByLabel('Note path',{exact:true}).fill(notePath);await page.getByLabel('Access',{exact:true}).selectOption('edit');
    await page.getByLabel('Password',{exact:true}).fill('1');
    const created=page.waitForResponse(r=>r.url()===origin+'/api/v1/shares'&&r.request().method()==='POST');
    await page.getByRole('dialog').getByRole('button',{name:'Create share',exact:true}).click();const share=await (await created).json(),shareUrl=share.url;await page.getByRole('dialog').waitFor({state:'detached'});assert.equal(await page.evaluate(()=>navigator.clipboard.readText()),shareUrl);
    await page.reload();await page.getByRole('searchbox',{name:'Search shared notes',exact:true}).fill(notePath);
    const row=page.locator(`[data-share="${share.share_id}"]`);await row.waitFor();await row.getByRole('button',{name:'Copy link',exact:true}).click();await page.getByText('Share link copied to the clipboard.',{exact:true}).waitFor();assert.equal(await page.evaluate(()=>navigator.clipboard.readText()),shareUrl);
    await page.evaluate(()=>{navigator.clipboard.write=async()=>{throw Error('Clipboard unavailable in this fixture');};});
    await row.getByRole('button',{name:'Copy link',exact:true}).click();await row.getByLabel('Share link',{exact:true}).waitFor();assert.equal(await row.getByLabel('Share link',{exact:true}).inputValue(),shareUrl);
    const visitor=await browser.newContext(),sharedPage=await visitor.newPage();await sharedPage.goto(shareUrl);await sharedPage.getByLabel('Share password',{exact:true}).fill('1');await sharedPage.getByRole('button',{name:'Enter',exact:true}).click();
    const editable=sharedPage.locator('#surface textarea');await editable.last().fill('\nVisitor workflow saved.\n');await sharedPage.getByText('Changes saved.',{exact:true}).waitFor();assert.ok((await api('/api/note?'+new URLSearchParams({path:notePath}))).text.includes('@root'));
    await page.bringToFront();await row.getByRole('button',{name:'Revoke',exact:true}).click();await page.getByRole('button',{name:'Revoke Share',exact:true}).click();await row.waitFor({state:'detached'});
    ...
    await nav('Crusher');await page.locator('input[type=file]').setInputFiles({name:'workflow-source.txt',mimeType:'text/plain',buffer:Buffer.from('SHELL_BROWSER_FIXTURE: Source material about links and hierarchical knowledge.')});await page.getByText('1 source accepted.',{exact:true}).first().waitFor({timeout:30000});
