...
(async()=>{
  ...
  try{
    ...
    await page.bringToFront();await row.getByRole('button',{name:'Revoke',exact:true}).click();await page.getByRole('button',{name:'Revoke Share',exact:true}).click();assert.equal((await visitor.request.get(shareUrl+'/api/policy')).status(),404);await visitor.close();results.shares_clipboard_public='PASS';
